...
public class ImportController {
    ...
    public Map<String, Object> importVbo(@RequestParam("file") MultipartFile file) throws Exception {
        Session session = importService.importVbo(file.getInputStream());
        String json = objectMapper.writeValueAsString(session);
        return Map.of("sessionId", session.getId(), "circuit", session.getCircuit(), "driver", session.getDriver(), "laps", session.getLaps());
    }
}
